...
func NewGRPCServer(c *conf.Server, inv *service.InventoryService, logger log.Logger,nacosClient *data.NacosClient) *grpc.Server {
	...
	v1.RegisterInventoryServer(srv, inv)
	...
	// 这里 订阅消息 监听库存归还topic
		...
		if err = rockClient.Subscribe("order_reback",consumer.MessageSelector{},func(ctx context.Context, me ...*primitive.MessageExt) (consumer.ConsumeResult, error) {
			return inv.AutoReback(ctx,me...)
		});err != nil{
			fmt.Println("读取消息失败")
		}
